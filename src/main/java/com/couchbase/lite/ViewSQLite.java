 ...
 */
public final class ViewSQLite implements View {
    ...
    protected ViewSQLite(Database database, String name) {
    ...
    public Cursor resultSetWithOptions(QueryOptions options) {
        ...
        if (maxKey != null) {
            maxKey = ViewUtil.keyForPrefixMatch(maxKey, options.getPrefixMatchLevel());
            if (inclusiveMax) {
                sql += " AND key <= ?";
            } else {
                sql += " AND key < ?";
     ...
     */
    @InterfaceAudience.Private
    protected List<QueryRow> reducedQuery(Cursor cursor, boolean group, int groupLevel) throws CouchbaseLiteException {
        ...
        while (!cursor.isAfterLast()) {
            ...
            if(group && !ViewUtil.groupTogether(keyObject, lastKey, groupLevel)) {
                if (lastKey != null) {
                    // This pair starts a new group, so reduce & record the last one:
                    Object reduced = (reduceBlock != null) ? reduceBlock.reduce(keysToReduce, valuesToReduce, false) : null;
                    Object key = ViewUtil.groupKey(lastKey, groupLevel);
        ...
        if(keysToReduce.size() > 0) {
            ...
            Object key = group ? ViewUtil.groupKey(lastKey, groupLevel) : null;
            Object reduced = (reduceBlock != null) ? reduceBlock.reduce(keysToReduce, valuesToReduce, false) : null;
            QueryRow row = new QueryRow(null, 0, key, reduced, null);
            row.setDatabase(database);
            rows.add(row);
